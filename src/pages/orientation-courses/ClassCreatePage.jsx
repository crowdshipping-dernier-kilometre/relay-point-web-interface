...
const ClassCreatePage = () => {
  ...
  const handleCreate = async () => {
    setIsLoading(true);
    const response = await orientationCourseService.createClass({
      name: values.name,
    ...
    dispatchToast("success", "Nouvelle classe créée");
    handleReset();
  };

  return (
    <div className="flex-1 overflow-auto relative z-10">
      <Header title={`Nouvelle classe`} />
      ...
      <main className="max-w-4xl mx-auto py-6 px-4 lg:px-8">
        ...
        <div className="flex flex-row justify-end space-x-4 mt-8">
          {isLoading ? (
            ...
            <Button
              variant="contained"
              disabled={!values.name || !values.description || !values.password}
              startIcon={<Add />}
